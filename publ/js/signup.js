...
const nicknameMessage = document.querySelector("div.nick-name-message");
...
function isValidPassword(event) {
    ...
    const passwordReg = /^(?=.*\d)(?=.*[a-zA-Z])[0-9a-zA-z]{8,20}$/; // 8~10자 영문, 숫자 조합
...
async function signup(event) {
    const isValidReq = isValidEmail() && isValidPassword() && isValidPasswordConfirm()
    && isValidNickname();
